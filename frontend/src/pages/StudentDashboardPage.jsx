...
function StudentDashboardPage() {
  ...
  const fetchNotifications = async () => {
    try {
      const response = await listComplianceNotifications();
  ...
  const handleOpenNotification = async (notification) => {
    ...
    try {
      await markComplianceNotificationRead(notification.id);
      setNotifications((previous) => previous.map((item) =>
        item.id === notification.id ? { ...item, isRead: true } : item
      ));
    } catch (error) {
      setErrorMessage(error.message || 'Unable to mark notification as read.');
    }
  };
  ...
  const handleSendReply = async (conversation) => {
    ...
    try {
      ...
      await sendComplianceMessage(conversation.id, `Re: ${firstMessage.subject}`, body);
      setConversationReply('');
      await fetchMessages();
    } catch (error) {
      setErrorMessage(error.message || 'Unable to send your reply.');
    } finally {
      setSendingMessageId('');
    }
  };
  ...
  const conversations = Object.values(messages.reduce((groups, message) => {
    ...
    if (!groups[conversationId]) {
      groups[conversationId] = {
        id: conversationId,
        officerEmail: message.senderUserId === String(currentUser?.id)
            ...
            <div className="profile-menu-dropdown student-inbox-dropdown">
              ...
              {notifications.length > 0 ? (
                <div className="student-notification-list">
                  {notifications.map((notification) => (
                    <button
                      type="button"
                      className={`student-notification ${notification.isRead ? '' : 'student-notification-unread'}`}
                      key={notification.id}
              ...
              <div className="student-inbox-compose">
                <select
                  ...
                  onChange={(event) =>
                    setNewMessage((previous) => ({ ...previous, requestId: event.target.value }))
                  }
                >
                ...
                <input
                  ...
                  onChange={(event) =>
                    setNewMessage((previous) => ({ ...previous, subject: event.target.value }))
                  }
                />
                <textarea
                  rows="3"
                  placeholder="Message compliance and press Enter"
                  value={newMessage.body}
                  onChange={(event) =>
                    setNewMessage((previous) => ({ ...previous, body: event.target.value }))
                  }
                  onKeyDown={(event) => {
                  ...
                  {activeConversation ? (
                    <div className="inbox-thread">
                      <div className="inbox-thread-header">
                        ...
                        <span>{activeConversation.officerEmail}</span>
                      </div>
          ...
          >
            ...
            {selectedComplianceRequest ? (
              <div>
                ...
                <h4 className="compliance-tasks-heading">Compliance Tasks</h4>

                {selectedComplianceRequest.guidelines?.length ? (
                  <div className="student-compliance-review">
                    {selectedComplianceRequest.guidelines.map((guideline) => (
                      <div className="guideline-review-item" key={guideline.id}>
                        <div className="guideline-review-heading">
                          <strong>{guideline.title}</strong>
                          <span className={`request-status-pill request-status-${guideline.decision || 'pending'}`}>
                            {guideline.decision === 'needs_changes'
                              ? 'Needs changes'
                              : guideline.decision || 'Pending'}
                          </span>
                        </div>
                        ...
                        <div className="student-guideline-feedback">
                          ...
                          {guideline.feedback ? (
                            <p>Officer feedback: {guideline.feedback}</p>
                          ) : null}
                        </div>
                      </div>
                    ))}
                  ...
                  <p>No compliance tasks recorded.</p>
                )}
              </div>
            ) : (
              <p>
