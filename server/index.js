...
const contractUpload = multer({
  ...
  fileFilter: (_req, file, callback) => {
    ...
    const hasPdfExtension = String(file.originalname || '')
      .toLowerCase()
      .endsWith('.pdf');
...
function buildUnsupportedSchoolEmailMessage() {
  const divisionSummary = getSupportedSchoolCounts()
    .map(
      ({ division, schoolCount }) =>
        `${schoolCount} schools in ${division}`
    )
    .join(', ');
...
function serializeUser(user, resolvedSchool) {
  const matchedSchool =
    resolvedSchool || resolveSchoolFromEmail(user.email);
...
function buildStoredFileName(contractId, originalName) {
  ...
  return `${contractId}-${sanitizedName
    .toLowerCase()
    .endsWith('.pdf')
    ? sanitizedName
    : `${sanitizedName}.pdf`}`;
...
function normalizeAgreementMetadata(metadata = {}) {
  ...
  return {
    ...
    deliverables: Array.isArray(metadata.deliverables)
      ? metadata.deliverables.map((item) => String(item || '').trim()).filter(Boolean)
...
async function hydrateLegacyContractMetadata(contract) {
  const existing = normalizeAgreementMetadata(
    contract.agreementMetadata
  );

  const needsExtraction =
    contract.agreementMetadataVersion !==
  ...
  try {
    ...
    const screening = await analyzeContractPdf(
      fileBuffer,
      contract.fileName
    );

    const merged = mergeExtractedAgreementMetadata(
      existing,
    ...
    await contractsCollection.updateOne(
      ...
      {
        $set: {
          agreementMetadata: merged,
          agreementMetadataVersion:
            AGREEMENT_METADATA_VERSION
        }
      }
    );
...
function serializeDocumentRequest(documentRequest) {
  return {
    ...
    contractFileName:
      documentRequest.contractFileName,
    contractFileSize:
      documentRequest.contractFileSize,
    studentUserId:
      documentRequest.studentUserId,
    studentEmail:
      documentRequest.studentEmail,
    studentSchool:
      documentRequest.studentSchool,
    studentDivision:
      documentRequest.studentDivision,

    assignedComplianceUserId:
      documentRequest.assignedComplianceUserId ||
      null,

    assignedComplianceEmail:
      documentRequest.assignedComplianceEmail ||
      null,
    ...
    reviewedBy:
      documentRequest.reviewedBy || null,

    reviewerEmail:
      documentRequest.reviewerEmail || null,

    hasSourceFile:
      Boolean(documentRequest.storedFilePath)
  };
}
...
function assertUserRole(user, ...roles) {
  ...
  const error = new Error(
    'This action is not permitted for the current user role.'
  );
...
function buildRosterGroups(buffer) {
  ...
  if (csvRows.length < 2) {
    const error = new Error(
      'The CSV must include a header row and at least one player row.'
    );
  ...
  const headerRow =
    csvRows[0].map(normalizeCsvHeader);
  ...
  csvRows.slice(1).forEach(
    (row, rowIndex) => {
      const player = {
        ...
        firstName: String(
          row[columnIndexes.first_name] || ''
        ).trim(),

        lastName: String(
          row[columnIndexes.last_name] || ''
        ).trim(),

        school: String(
          row[columnIndexes.school] || ''
        ).trim(),

        sport: String(
          row[columnIndexes.sport] || ''
        ).trim(),

        year: String(
          row[columnIndexes.year] || ''
        ).trim()
      };
      ...
      if (hasMissingValue) {
        const error = new Error(
          `Row ${rowIndex + 2} contains blank required values.`
        );
      ...
      const groupKey =
        `${player.school.toLowerCase()}::` +
        `${player.sport.toLowerCase()}::` +
        `${player.year.toLowerCase()}`;
...
function serializeRoster(roster) {
  return {
    ...
    hasSourceFile:
      Boolean(roster.storedFilePath),
    createdAt: roster.createdAt,
    updatedAt: roster.updatedAt
  };
}

async function deleteRosterFileIfUnused(
  storedFilePath
) {
  if (!storedFilePath) {
    return;
  ...
  const remainingReferenceCount =
    await rostersCollection.countDocuments({
      storedFilePath
    });
  ...
  async (req, res, next) => {
    try {
      ...
      const hydratedContracts =
        await Promise.all(
          contracts.map(
            hydrateLegacyContractMetadata
          )
        );

      res.json({
        contracts:
          hydratedContracts.map(
            serializeContract
          )
      });
    } catch (error) {
  ...
  async (req, res, next) => {
    try {
      const {
        userId,
        user
      } = await getRequiredCurrentUser(req);
      ...
      if (user.school) {
        const normalizedAssignedSchool =
          user.school
            .toLowerCase()
            .trim();

        const hasMismatchedSchool =
          rosterGroups.some(
            (rosterGroup) =>
              rosterGroup.school
                .toLowerCase()
                .trim() !==
              normalizedAssignedSchool
          );

        if (hasMismatchedSchool) {
          return res.status(403).json({
            message:
              `Roster uploads for this account must match the assigned school: ${user.school}.`
          });
        }
      }
      ...
      const rosterUploadId =
        `RST-${Date.now()}-${randomUUID()
          .slice(0, 8)
          .toUpperCase()}`;

      const userUploadDirectory =
        path.join(
          rosterUploadsRoot,
          userId
        );
      ...
      for (
        const rosterGroup of rosterGroups
      ) {
        const rosterId =
        ...
        const existingRoster =
          await rostersCollection.findOne({
            userId,
            school:
              rosterGroup.school,
            sport:
      ...
      for (
        const staleFilePath of staleFilePaths
      ) {
        await deleteRosterFileIfUnused(
          staleFilePath
        );
      }
      ...
      return res.status(201).json({
        message:
          `Processed ${savedRosters.length} roster ${
            savedRosters.length === 1
              ? 'group'
              : 'groups'
          } from CSV upload.`,
    ...
    try {
      ...
      if (!roster) {
        return res.status(404).json({
          message:
            'Roster not found for the current user.'
        });
      }
      ...
      if (!roster.storedFilePath) {
        return res.status(404).json({
          message:
            'No CSV file is stored for this roster.'
        });
      }
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="${roster.sourceFileName || 'roster.csv'}"`
      );

      return res.sendFile(
        roster.storedFilePath
      );
    } catch (error) {
      next(error);
    }
...
app.delete(
  ...
  async (req, res, next) => {
    try {
      ...
      await rostersCollection.deleteOne({
        rosterId,
        userId
      });
...
app.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const contractScreening =
        await analyzeContractPdf(
          file.buffer,
          file.originalname
        );

      if (
        !contractScreening.isContract
      ) {
        return res.status(400).json({
          code:
            'NOT_A_CONTRACT',
          ...
          message:
            'Upload rejected: this PDF does not appear to be a contract.'
        });
      }

      const contractId =
        `CTR-${Date.now()}-${randomUUID()
          .slice(0, 8)
          .toUpperCase()}`;
      ...
      const contractDocument = {
        ...
        aiScreening: {
          score:
            contractScreening.score,

          positiveSignals:
            contractScreening.positiveSignals,

          negativeSignals:
            contractScreening.negativeSignals,

          textLength:
            contractScreening.textLength,
 ...
 * ============================================================
 *
 * This is the endpoint that the updated contractApi.js calls:
 *
 * PATCH /api/contracts/:contractId/metadata
 *
 * It persists:
 * - Brand / Payer
 * - Contract Value
 ...
 * - Amendments
 *
 * This data is stored inside the contract document in MongoDB.
 */
app.patch(
  ...
  async (req, res, next) => {
    try {
      ...
      const contract =
        await contractsCollection.findOne({
          contractId,
          userId
        });
      ...
      if (!contract) {
        return res.status(404).json({
          message:
            'Contract not found for the current user.'
        });
      }
       ...
       * Amendments
       *
       * Keep both date and notes.
       * Invalid completely-empty entries
       * are removed.
       */
      const amendments =
        Array.isArray(
      ...
      /*
       * Build one normalized metadata object.
       *
       * If a field isn't supplied by the
       * frontend, preserve the value already
       * stored in MongoDB.
       */
      const metadata = {
        athleteName:
  ...
  async (req, res, next) => {
    try {
      ...
      if (user.role === 'student') {
        const requests =
          await documentRequestsCollection
            .find({
  ...
  async (req, res, next) => {
    try {
      ...
      if (!complianceEmail) {
        return res.status(400).json({
          message:
            'A compliance officer email is required.'
        });
      }
      ...
      if (!contract) {
        return res.status(404).json({
          message:
            'Contract not found for the current student.'
        });
      }
      ...
      if (!complianceOfficer) {
        return res.status(404).json({
          message:
            'No compliance officer account with that email was found for your school.'
        });
      }
      ...
      await documentRequestsCollection.updateOne(
        ...
        },
        {
          $set: {
            ...
            assignedComplianceUserId:
              complianceOfficer._id.toString(),
      ...
      return res.status(
        ...
        message:
          `Document submitted to ${complianceOfficer.email} for review.`,
  ...
  async (req, res, next) => {
    try {
      ...
      if (!documentRequest) {
        return res.status(404).json({
          message:
            'Compliance request not found for the current officer.'
        });
      }
...
app.get(
  '/api/compliance/requests/:requestId/file',
  async (req, res, next) => {
    try {
      const {
        userId,
      ...
      if (
        !documentRequest.storedFilePath
      ) {
        return res.status(404).json({
          message:
            'No document file is stored for this request.'
        });
      }

      res.setHeader(
        'Content-Type',
        documentRequest.mimeType ||
          'application/pdf'
      );

      res.setHeader(
        'Content-Disposition',
        `inline; filename="${documentRequest.contractFileName || 'document.pdf'}"`
      );
...
app.post(
  ...
  async (req, res) => {
    ...
    if (!resolvedSchool) {
      return res.status(400).json({
        message:
          buildUnsupportedSchoolEmailMessage()
      });
    }
    ...
    const insertResult =
      await usersCollection.insertOne({
        ...
        ncaaDivision:
          resolvedSchool.division,

        schoolEmailDomain:
          resolvedSchool.primaryDomain,
...
app.post(
  ...
  async (req, res) => {
    ...
    if (
      ...
      return res.status(403).json({
        message:
          `This account is registered as ${user.role}, not ${expectedRole}.`
      });
    }
...
app.delete(
  ...
  async (req, res, next) => {
    try {
      ...
      await contractsCollection.deleteOne({
        contractId,
        userId
      });
      ...
      if (
        ...
        const remaining =
          await contractsCollection.countDocuments(
            {
              storedFilePath:
                contract.storedFilePath
            }
...
async function start() {
  ...
  await rostersCollection.createIndex(
    {
      userId: 1,
      updatedAt: -1
    }
  ...
  await documentRequestsCollection.createIndex(
    {
      requestId: 1
    },
    {
  ...
  app.get(
    ...
    async (req, res, next) => {
      try {
        ...
        if (!contract) {
          console.log(
            'Contract not found for analysis:',
            {
              contractId,
              userId
            }
        ...
        if (
          ...
          console.log(
            'Contract file not found for analysis:',
            {
              contractId,
              userId
            }
        ...
        res.json({
          ...
          summary: {
            ...
            division:
              contract.ncaaDivision,

            stateName:
              contract.schoolStateName
          },
...
app.use(
  (
    ...
    if (
      ...
      return res.status(400).json({
        message:
          'File uploads exceed the allowed size limit.'
      });
    }
